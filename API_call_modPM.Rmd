...
process_sensor <- function(sensor) {
  time_range <- seq(as.Date(sensor@startDate), as.Date(sensor@endDate), by = "day")
  time_range <- format(time_range, "%Y-%m-%d")

  for (date in time_range) {
    merged_data_per_day <- data.frame()
    for (state in c("raw/", "")) {
      sensor_url <- paste0(sensor@sensorNum, "/data-by-date/", state, date, "/")
      api_data <- GET(
    ...
    if (is.null(raw_df) || is.null(final_df) || nrow(raw_df) == 0 || nrow(final_df) == 0) {
      next
    }
  ...
  cols_to_unnest <- names(merged_data_per_day)[sapply(merged_data_per_day, is.data.frame)]
  flattened_df <- unnest(merged_data_per_day, cols = cols_to_unnest, names_sep = ".")
  ...
  tryCatch({
    write.csv(flattened_df, csv_file_path)
  },
  error = function(e) {
    cat(
      paste0(
