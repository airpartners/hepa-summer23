...
all_df <- all_df %>%
  # Classify all the deployments
  mutate(beta_room_type = case_when(
    # All residential deployments
    area == "Roxbury" ~ "Roxbury-residential",
    place_id == "MF" & participant_id != 6 ~ "EB-residential",
    
    # All classroom deployments
    area == "Revere" & str_detect(participant_id, "class") ~ "R-classroom",
    
    # All office deployments
    area == "Revere" & !str_detect(participant_id, "class") ~ "R-office",
    
    # All daycare deployments
    place_id %in% c("LF", "SS") & participant_id != "kitchen" ~ "EB-daycare",
    place_id == "MF"  & participant_id == "6" | 
      participant_id == "kitchen" ~ "EB-kitchen"))
```
...
# Calculate summary statistics for each room type
beta_class_summary <- main_df_long %>%
  select(-room) %>%
  drop_na() %>%
  # Group by participant ID, case, environment, and type
  group_by(beta_room_type, case, environment, type) %>%
  # Calculate summary statistics
  my_summarise() %>%
  # Add participant ID column as 'all'
  mutate(participant_id = "all", area = "all", place_id = "all", 
...
overall_summary <- main_df_long %>%
  ...
  my_summarise() %>%
  # Add participant ID column as 'all'
  mutate(room_type = "all", beta_room_type = "all", participant_id = "all", 
         area = "all", place_id = "all")
```

```{r}
# Bind together participant and overall summary statistics
summary <- rbind(participant_summary, class_summary, beta_class_summary, overall_summary)
```
