...
Results can be found in `summary/Community_All/s_All_MC_quants.csv`.
...
for (file_path in modpm_data) {
  ...
  main_df <- main_df %>%
    ...
    mutate(case = case_when(case == "after" ~ "on", case == "before" ~ "off",
                            .default = case),
           dep_id = dep_vec[[1]][2]) %>%
    # Add in the deployment classification
    mutate(dep_class = case_when(dep_vec[[1]][1] == "Roxbury" ~ "residential",
                                 dep_vec[[1]][2] == "MF" ~ "residential",
                                 dep_vec[[1]][1] == "EastBoston" ~ "preschool",
                                 dep_vec[[1]][2] == "CH" ~ "municipal",
                                 dep_vec[[1]][2] == "RH" ~ "school"))
...
```{r}
# Bind two dataframes
all_df <- rbind(modpm_cpc_df, modpm_df)
```
...
participant_summary <- main_df_long %>%
  ...
  group_by(participant_id, dep_class, case, environment, type) %>%
  # Calculate summary statistics
  my_summarise()

# Calculate summary statistics for each deployment class
class_summary <- main_df_long %>%
  select(-room) %>%
  drop_na() %>%
  # Group by participant ID, case, environment, and type
  group_by(dep_class, case, environment, type) %>%
  # Calculate summary statistics
  my_summarise() %>%
...
overall_summary <- main_df_long %>%
  ...
  my_summarise() %>%
  # Add participant ID column as 'all'
  mutate(dep_class = "all", participant_id = "all")
```
