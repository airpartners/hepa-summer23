...
```{r}
get_plumes <- function(before, after, id) {
  # Remove missing data
  indices <- which(my_df_before$pm25 > 0)
  beforepm25 <- my_df_before$pm25[indices]
...
```{r}
plume_redus <- function(plumes, id) {
  decay_sum <-
    plumes %>%
      group_by(hepa) %>%
      summarize(
...
for (id in unique(my_df$participant_id)) {
    ...
    new_plumes <- get_plumes(my_df_before, my_df_after, id)
    new_reducs <- plume_redus(new_plumes, id)
