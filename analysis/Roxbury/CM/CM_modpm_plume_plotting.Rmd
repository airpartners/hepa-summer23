...
get_plumes <- function(sub_data, id, pm) {
  ...
  return(data.frame(all_decays[,c(10, 11, 12, 13, 14, 1:9)]))
}
```
...
plume_redus <- function(plumes, id, pm) {
  ...
  return(data.frame(decay_sum[,c(16, 17, 18, 1:15)]))
}
```
...
ggplot(exposure, aes(x = factor(participant_id), y = avg_plume, fill = factor(hepa))) +
  geom_bar(stat = "identity", position = "dodge") +
