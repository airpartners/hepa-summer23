...
pm25_indoor <- graph_main_df[graph_main_df$environment=='indoor' & graph_main_df$particle=='pm10',]
...
```{r}
# Get data subset
my_df_subset <- my_df[my_df$participant_id=="210" & my_df$environment=="indoor",]
...
# Remove missing data
indices <- which(my_df_before$pm10 > 0)
beforepm25 <- my_df_before$pm10[indices]

indices <- which(my_df_after$pm10 > 0)
afterpm25 <- my_df_after$pm10[indices]
...
```{r}
plume_redus <- function(sub_data, id, pm) {
...
for (id in unique(my_df$participant_id)) {
  for (pm in c("pm1", "pm25", "pm10")) {
    ...
    plume_reduc.newdata <- data.frame(
      ...
      "pv_peak_wdh_mean" = res$pv_peak_wdh_mean,
      "pv_peak_wdh_median" = res$pv_peak_wdh_median,
      "total_peak_wdh_mean" = res$total_peak_wdh_mean,
      "total_peak_wdh_median" = res$total_peak_wdh_median,
...
Save 
```{r}
write.csv(plume_reduc.data, file = paste0("summary/Roxbury/CM/", "s_CM_M_plumes.csv"))
```

## Visualizations for Plume Plotting

These are some candidate visualizations for presenting the information
pertaining to plume statistics. Eventually these will be ported over to 
Community All; until then, we will use IB data to display what these
visualizations will encapsulate and their strengths and limitations.
...
### Average Plume Exposure for Each Participant
```{r}
exposure <- plume_reduc.data %>%
  select(participant_id, pm, hepa, total_plume_area, n) %>%
  mutate(total_plume_area = total_plume_area/n) %>%
  group_by(participant_id, hepa) %>%
  mutate(sum_plume = sum(total_plume_area), .keep="none") %>%
