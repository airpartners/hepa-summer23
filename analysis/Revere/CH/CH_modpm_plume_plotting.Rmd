...
Plot various plumes before and after HEPA instalation to compare their
...
```{r}
# Get data subset
my_df_subset <- my_df[my_df$participant_id=="clerk" & my_df$environment=="indoor",]
...
points(peaks_b[, 3], my_df_before_smoothed$pm25[peaks_b[, 3]], pch = 20, col = "yellow")
...
# points(peaks_a[, 4], my_df_after_smoothed$y[peaks_a[, 4]], pch = 20, col = "green")
```
...
```{r}
plume_redus <- function(data, id, pm, env) {
  ...
  my_df_after_smoothed <- lowess(my_df_before[[pm]], f=0.001)
...
for (id in unique(my_df$participant_id)) {
  for (pm in c("pm1", "pm25", "pm10")) {
    for (env in c("indoor")) {
      ...
      res <- plume_redus(my_df, id, pm, env)
...
write.csv(plume_reduc.data, "summary/Revere/CH/s_CH_M_plumes.csv")
```
