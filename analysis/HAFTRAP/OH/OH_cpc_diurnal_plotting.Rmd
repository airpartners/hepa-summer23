...
main_main_df <- filter(main_main_main_df, participant_id %in% participants)
```
...
graph_main_df <- summary_dataframe(main_df)
```
...
diurnal_plot <- function(data, case_str, env_str) {
  ...
  graph_labels <- c("Daily", str_to_title(env_str), "PM", "Trends", "During", str_to_title(case_str), "Period", "[ ug/m", "]", "Concentration", "CPC")
  ...
  ggplot(data) +
    ...
    labs(title = bquote(.(graph_labels[11]) ~ .(graph_labels[1]) ~ .(graph_labels[2]) ~ .(graph_labels[3]) ~ .(graph_labels[4]) ~ .(graph_labels[5]) ~ .(graph_labels[6]) ~ .(graph_labels[7])),
         x = "Time of Day (hrs)",
         y = bquote(.(graph_labels[3]) ~ .(graph_labels[10]) ~ .(graph_labels[8])^3 ~ .(graph_labels[9]))) + 
...
diurnal_plot(graph_main_df, "hepa", "indoor")
diurnal_plot(graph_main_df, "hepa", "outdoor")
diurnal_plot(graph_main_df, "hepa", "ratio")

diurnal_plot(graph_main_df, "sham", "indoor")
diurnal_plot(graph_main_df, "sham", "outdoor")
diurnal_plot(graph_main_df, "sham", "ratio")
```
