...
for (case in c("sham", "hepa")) {
  ...
  for (person in participants) {
    ...
    main_df_wide %>%
      # Filter for case
      filter(participant_id == person, case == case) %>%
      # Get correlation coefficients (look at helper function)
      get_corr(case = case, person = person) -> df
  ...
  main_df_wide %>%
    ...
    filter(case == case) %>%
    # Get correlation coefficients (look at helper function)
    get_corr(case = case, person = "all") -> df
  
  # Append to main dataframe
  corr_df <- rbind(corr_df, df)
  print(paste("Computed correlation coefficients for all", case))
}
