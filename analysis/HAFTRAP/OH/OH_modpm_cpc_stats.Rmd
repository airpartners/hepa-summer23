...
When correlating Mod-PM and CPC data, we want to make sure that the CPC data - sampled ever second, is at the same sampling rate as Mod-PM data - sampled every minute. This function calculates the one minute average for CPC data.
```{r}
# Round over 10 minute averages
round_1min <- function(df) {
  df %>%
...
cpc_df <- main_df %>%
  # Mod-PM data is sampled by whole minute, average CPC data to match
  round_1min()
