...
This script is used to calculated summary statistics for multiple participants in the HAFTRAP study, for CPC deployments by Olin. It is largely a copy of `OH_modpm_single_stats.Rmd` (found in the `testing`folder) without the data cleaning part found in `OH_modpm_cleaning`.

Results can be found in the folder`summary/HAFTRAP/OH/`.
...
write.csv(corr_df, "summary/HAFTRAP/OH/s_OH_M_corr.csv")
```