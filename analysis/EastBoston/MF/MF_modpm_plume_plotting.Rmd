...
pm25_indoor <- graph_main_df[graph_main_df$environment=='ratio' & graph_main_df$particle=='pm1',]
...
```{r}
# Get data subset
my_df_subset <- my_df[my_df$participant_id=="2" & my_df$environment=="ratio",]
...
write.csv(plume_reduc.data, "summary/EastBoston/MF/s_MF_M_plumes.csv")
```
...
exposure <- plume_reduc.data %>%
  select(participant_id, pm, hepa, total_plume_area) %>%
