...
```{r}
# Get data subset
my_df_subset <- my_df[my_df$participant_id=="10" & my_df$environment=="indoor",]
...
points(peaks_a[, 4], my_df_after$pm25[peaks_a[, 4]], pch = 20, col = "green")
```
...
ggplot(plume_reduc.data, aes(x = factor(pm), y = abs_k_val, fill = factor(hepa))) +
  ...
  coord_cartesian(ylim = quantile(plume_reduc.data$abs_k_val, c(0.0, 0.99))) +
  theme_minimal()
...
```{r}
exposure <- plume_reduc.data[plume_reduc.data$pm=="pm10",] %>%
  select(participant_id, hepa, total_plume_area, n) %>%
