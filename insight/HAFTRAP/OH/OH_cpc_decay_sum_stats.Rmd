# HAFTRAP Summary Statistics for Decay Data (for CPC)

## STOP

**HAVE YOU RUN `OH_cpc_decay_analysis`?**
*This file loads a dataframe created from running `OH_cpc_decay_analysis`. Make sure you run that file first (if you haven't already) before running this file.*

This file is used to calculate summary statistics from the decay constants for the HAFTRAP CPC data.
...
decay_constants <- read.csv(paste0(path_to_data, "OH_C_decay.csv"))
```
...
```{r}
# Spread: Create separate sham and hepa summary statistics columns
...
dir.create("artifacts/decay")
```

```{r}
# Save final decay summary dataframe to CSV
write.csv(summary_normal, "artifacts/decay/OH_C_decay_summary.csv")
```
