# HAFTRAP Summary Statistics for Decay Data (for Mod-PM)

## STOP

**HAVE YOU RUN `OH_modpm_decay_analysis`?**
*This file loads a dataframe created from running `OH_modpm_decay_analysis`. Make sure you run that file first (if you haven't already) before running this file.*

This file is used to calculate summary statistics from the decay constants for the HAFTRAP Mod-PM data.
...
decay_constants <- read.csv(paste0(path_to_data, "OH_M_decay.csv"))
```
...
write.csv(summary_normal, "artifacts/decay/OH_M_decay_summary.csv")
```
