...
summary_df %>%
  filter(place_id == "IB", participant_id == "105", 
         environment == "indoor", type == "pm25", case != "redu") -> sum_stats
...
all_df %>% 
  filter(place_id == "IB", participant_id == "105", 
         environment == "indoor") %>% 
  left_join(sum_stats) %>%
  mutate(quartile = case_when(pm25 < q5 ~ "< 5th",
                   pm25 > q5 & pm25 < q25 ~ "5th to 25th",
                   pm25 > q25 & pm25 < median ~ "25th to Median",
                   pm25 > median & pm25 < q75 ~ "Median to 75th",
                   pm25 > q75 & pm25 < q95 ~ "75th to 95th",
                   .default = "> 95th")) %>%
  ggplot(aes(x = date, y = pm25, color = quartile)) +
    geom_point(size = 0.1) +
...
summary_df %>%
  filter(place_id == "IB", participant_id == "105", 
         environment == "indoor", type == "pm25", case == "off") -> ss_off
...
plot_df %>% 
  ggplot() +
    geom_line(aes(x = date, y = pm25), size = 0.1) +
