...
mean_df <- select_df %>% 
    group_by(size_bin, case) %>% 
    summarize(proportion = mean(proportion)) %>%
    mutate(place_id = "*Mean")
...
```{r}
select_df <- all_df %>%
  proportion_pm1num(c("case", "place_id"), 
               env_vars = c("indoor"), case_vars = c("off", "on"))
...
```{r}
select_df <- all_df %>%
  proportion_pm1num(c("environment", "place_id"), 
               env_vars = c("indoor", "outdoor"), case_vars = c("off", "on", NA))
...
```{r}
select_df <- all_df %>%
  proportion_pm1num(c("environment", "area"), 
               env_vars = c("indoor", "outdoor"), case_vars = c("off", "on", NA))
...
plot_df <- rbind(select_df, mean_df)  %>%
  mutate(area = case_when(area == "EastBoston" ~ "East Boston", .default = area))

plot_df %>%
  ggplot(aes(x = area, y = proportion, fill = size_bin)) +
    geom_bar(stat = "identity") +
    facet_wrap(vars(environment), ncol = 2) +
    plot_label(plot_df) +
    scale_fill_discrete(
...
```{r}
propnum_plot +
  labs(x = "Area of Boston", y = "Proportion of Total Count", 
       title = bquote("Proportion of"~PM[1]~"by Particle Count")) +
  geom_vline(xintercept = 1.5, linetype = "dashed", color = "#666666")
```
